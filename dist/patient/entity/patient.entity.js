"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PatientEntity = void 0;
const entry_entity_1 = require("../../entry/entities/entry.entity");
const evolution_entity_1 = require("../../evolution/entities/evolution.entity");
const user_entity_1 = require("../../users/entities/user.entity");
const typeorm_1 = require("typeorm");
let PatientEntity = class PatientEntity {
};
__decorate([
    (0, typeorm_1.PrimaryGeneratedColumn)(),
    __metadata("design:type", Number)
], PatientEntity.prototype, "id", void 0);
__decorate([
    (0, typeorm_1.Column)({ length: 50 }),
    __metadata("design:type", String)
], PatientEntity.prototype, "firstName", void 0);
__decorate([
    (0, typeorm_1.Column)({ length: 50 }),
    __metadata("design:type", String)
], PatientEntity.prototype, "lastName", void 0);
__decorate([
    (0, typeorm_1.Column)({ unique: true }),
    __metadata("design:type", String)
], PatientEntity.prototype, "idCard", void 0);
__decorate([
    (0, typeorm_1.Column)('date'),
    __metadata("design:type", Date)
], PatientEntity.prototype, "dob", void 0);
__decorate([
    (0, typeorm_1.Column)(),
    __metadata("design:type", String)
], PatientEntity.prototype, "contactNumber", void 0);
__decorate([
    (0, typeorm_1.Column)(),
    __metadata("design:type", String)
], PatientEntity.prototype, "email", void 0);
__decorate([
    (0, typeorm_1.ManyToOne)(() => user_entity_1.UserEntity, (user) => user.patients),
    __metadata("design:type", user_entity_1.UserEntity)
], PatientEntity.prototype, "user", void 0);
__decorate([
    (0, typeorm_1.OneToMany)(() => entry_entity_1.EntryEntity, (entry) => entry.patient, {
        onDelete: 'CASCADE',
    }),
    __metadata("design:type", Array)
], PatientEntity.prototype, "medicalRecord", void 0);
__decorate([
    (0, typeorm_1.OneToMany)(() => evolution_entity_1.EvolutionEntity, (evolution) => evolution.patient, {
        onDelete: 'CASCADE',
    }),
    __metadata("design:type", evolution_entity_1.EvolutionEntity)
], PatientEntity.prototype, "evolution", void 0);
PatientEntity = __decorate([
    (0, typeorm_1.Entity)({ name: 'patient' })
], PatientEntity);
exports.PatientEntity = PatientEntity;
//# sourceMappingURL=patient.entity.js.map